...
## 20. ライトユーザー動線 ②: タスクテンプレートと専門家層の表示切替(2026-09-16)

- `unirobolab task-set <contract> --train <train.json> [--goal-range r | --goal-min a --goal-max b]
  [--tolerance t] [--time s]`: 学習設定のうちライトユーザーが決める項目だけを物理量で書き換える
  (制限時間は契約の周期からステップ数に換算、許容誤差は早期終了の閾値か到達半径)。報酬や
  PPO の設定には触れない。`task-show` は同じ項目を 1 行の JSON で返す(GUI が読み戻す)。
- 本体の UniRoboLab パネルは **Task タブが既定**になり、契約から種類(固定基体 → 関節目標、
  移動基体 → 地点到達)を判定して、目標範囲(rad または最短・最長距離 m)、成功の許容誤差、
...
- **行動の増分モード** (`action_term.relative`): 絶対位置の関節目標 (scale ±1 rad) だと探索の乱雑な掃いで
  物体を弾いてしまい、40 万ステップで成功率 1 % 未満だった。増分 (前回の目標 + 最大 0.06 rad/ステップ、
  joint_limits でクランプ) にすると同じ 40 万ステップで成功率 71 %、平均誤差 4.9 cm (許容 6 cm)。
  学習環境・live・policy_node の 3 経路で同じ積分。policy_node は安全停止からの復帰で計測位置から
  やり直し、計測位置と積分目標が 0.3 rad 以上離れたら (手で動かされた・リセット) 計測位置に再同期する。
- **通した結果** (arm2 fixture、押す): ④ live で 6.7 cm、⑤ ROS 2 経路で 3 目標中 2 つ合格 (6.7 / 10.2 / 3.9 cm、
  許容 9 cm)、非常停止試験 PASS。残る 1 つは方策の精度 (成功率 71 % のまま学習を終えた) の問題。
- ⑤ で踏んだ落とし穴と対処:
  - policy_node は目標が届く前に動いてはいけない: 空間目標 (base / link / object) の初期値 0 は「原点へ」
    という実在の目標になる。物体が (10, 10) の待機位置にある間に腕が折り畳まれ、そこから回復できなかった。
    → 空間目標の観測を持つ契約では goal を受け取るまで指令しない (`needs_goal`)。物体の位置も同様。
  - 目標ごとに開始状態へ戻す: 方策は開始姿勢 (関節 0) からしか学習していないので、2 つ目以降の目標は
    前の目標で終わった姿勢からでは失敗する。sim2sim は物体タスクで各目標の前に reset_simulation
    (SCOPE_STATE) を呼ぶ。ただしノードは古い観測から最後の目標を送り続けるので、リセットの前後を
    非常停止で挟む (停止 → リセット → 物体配置 → 解除 → ramp_in 待ち)。
  - URDF に joint_states_topic が無いロボットは、シミュレータが `/joint_states` `/joint_command`
    (名前空間なし) を使うのに契約は `/<ns>/...` を期待する。check_runner は URDF にトピック名が無ければ
    entity_namespace 付きで spawn_entity サービスを直接呼ぶ (トピック名がある URDF に名前空間を付けると
    二重になるので付けない)。spawn_entity の座標パラメータは DOUBLE 型なので整数を渡せない。
- 未対応・課題: 把持 (物体を持ち上げる・離す)、物体の向きの条件、複数物体、移動基体での物体タスク、
  実機での物体位置の推定 (カメラ・マーカ) はユーザーの用意。開始姿勢のばらつき (関節をランダムな
  姿勢から始める) を学習に入れないと、途中の姿勢から目標を与えたときに回復できない。

## 28. 開始姿勢と物理のばらつき、履歴窓の方策 (domain randomization、2026-09-19〜20)

「途中の姿勢から目標を与えると回復できない」(§27) への対応と、実機とのずれ (物体の質量・摩擦、
駆動の応答) に備えるための土台。ユーザーの提案は「DR が一般的だが context-aware adaptation が
できるとより良い」。整理: 開始姿勢は観測に入るので状態側の DR で足りる。適応が効くのは観測に
現れない量 (質量・摩擦・ゲイン) で、まず履歴窓の方策 (直近の観測・行動から状況を推定) から入り、
足りなければ RMA 型 (特権情報のエンコーダ + 履歴からの適応モジュール) へ進む、という順にした。

- **学習サーバの op**: SET_JOINTS (op 9、関節を直接置く。共有シーンで「目標を送って整定を待つ」と他の
  env の物理も進むため) と SET_DYNAMICS (op 10、質量・摩擦・駆動ゲインをスポーン時の値に対する倍率で
  変える)。学習環境と live は開始時に全エンティティを公称値へ戻す (前回の DR が残る事故を踏んだ)。
- **task.json**: `start.joints = random` + `joints_fraction` (可動範囲の割合)、`training.randomize`
  {object_mass, object_friction, drive_gain}、`training.history_length`。GUI ② に「開始の関節姿勢」
  「物理のばらつき」「方策の履歴窓」の選択。
- **カリキュラム** (train.py `_StartCurriculum`): 開始姿勢の幅を 0 から始め、直近の窓の成功率が 0.5 を
  超えるたびに 0.05 ずつ設定値まで広げる。いきなり幅 25 % で始めた学習は 41 万ステップで成功率 13 %
  のまま立ち上がらなかった (開始ゼロで学習した方策をその幅で評価すると 50 %)。
- **途中保存と最良の方策** (train.py): 5 万ステップごとに model と ONNX を保存。policy.onnx は
  直近の窓の成績が最も良かった時点 (policy_last.onnx が最後)。PPO は終盤に崩れることがあり、
  物理ばらつき付きの 1 本は窓の成功率 53 % → 28 % に落ち、最後の方策は評価で 6 % だった。
- **評価コマンド** `unirobolab eval`: 方策を学習環境で走らせ、`--task-override` で開始姿勢や物理の
  ばらつきを上書きして成功率と最終誤差を出す。
- **結果** (arm2 で押す、各 120 エピソード、公称 = 開始ゼロ・公称物理、ばらつき = 開始 25 %・質量
  ×0.5〜2・摩擦 0.2〜1.0・駆動 ×0.7〜1.3):

  | 学習条件 (すべて増分行動) | 公称 | ばらつき |
  |---|---|---|
  | 開始ゼロ (40 万) | 62.5 % | 50.8 % |
  | 開始ランダム 25 % + カリキュラム (80 万) | 43.3 % | 30.0 % |
  | 同上 + 物理ばらつき (80 万) | 5.8 % (終盤に崩壊) | 10.0 % |
  | 同上 + 物理ばらつき + 履歴窓 8 (80 万) | **70.8 %** | **48.3 %** |

  履歴窓 8 + 物理ばらつきの方策が公称で最も良く、ばらつき下でも開始ゼロの方策と同等。開始姿勢の
  ランダム化そのものは、この 2 自由度の腕では物体の後ろへ回り込めない開始が混ざるため成功率の上限を
  下げる (開始ゼロの方策も幅 25 % では 50 %)。幅は 25 % より小さい既定 (10〜15 %) が妥当。
