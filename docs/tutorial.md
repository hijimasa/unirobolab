# チュートリアル: URDF から実機用パッケージまで (servo_demo)

UniRoboLab の画面だけで、ロボットの URDF から「学習 → 試す → 配備前チェック → 実機向けの ROS 2
パッケージ」まで通す手順です。題材は 2 関節のサーボ台 (servo_demo)。関節を目標角へ動かす方策を
学習します。所要時間はおよそ 20 分 (うち学習が 7〜8 分)。

画面は 2026-09-17 に実際に通したときのものです (v1 の GUI での旧版は docs/tutorial-v1-2026-09-16.md)。

## 0. 準備 (一度だけ)

- プレイヤーを用意する: リリースの zip を展開すると `player/` に入っています (zip 内の README 参照)。
  ソースから作るなら `scripts/build_player.sh` → `generated/player/UniRoboLab.x86_64`
- Python 環境を作る: `scripts/setup_python.sh --training` (`.venv` ができ、画面が自動で見つけます。
  Windows は `powershell -ExecutionPolicy Bypass -File scripts\setup_python.ps1 -Training`)
- 配備前チェック (⑤) には ROS 2 が要ります。この PC に無ければ Docker を入れておけば、⑤ の画面の
  「コンテナを起動」(または `scripts/sim2sim_container.sh start`) が ROS 2 のコンテナを用意します
  (初回はイメージの作成に 10 分ほど)

起動:

```bash
scripts/unirobolab_gui.sh ~/unirobolab_projects/servo   # 引数はプロジェクトのフォルダ (無ければ作られる)
# リリース版: ./UniRoboLab.sh ~/unirobolab_projects/servo (Windows: UniRoboLab.cmd %USERPROFILE%\unirobolab_projects\servo)
```

設定ファイルを書く必要はありません。学習用のサーバは既定で有効です。

表示は日本語と英語を切り替えられます。右上の「English」(英語表示のときは「日本語」) を押すと、
画面を組み直して切り替わります。選んだ言語は覚えるので、次に起動したときもその言語で出ます。
学習中に切り替えても、学習は止まりません。

表示は日本語と英語を切り替えられます。右上の「English」(英語表示のときは「日本語」) を押すと、
画面を組み直して切り替わります。選んだ言語は覚えるので、次に起動したときもその言語で出ます。
学習中に切り替えても、学習は止まりません。

画面は上から「ステッパー (① ロボット → ⑥ 実機へ)」「左: そのフェーズの主画面、右: 3D」「下: 状態行と
詳細 / 戻る / 次へ」です。ステッパーは今いる段階を青、済んだ段階を緑で示し、前提が無い段階は押せません。
やり直しが必要になった段階 (上流を変えたとき) には「!」が付きます。

## ① ロボット

「選ぶ...」で URDF を選びます。関節と可動範囲、指令の方式、基体の種類が読み取られて表に出て、
右にロボットが現れます。

![① ロボット](images/tutorial2/01_robot.png)

下の「実機の接続先」は、⑥ で作るパッケージがそのまま使う値です。名前空間 (`/ServoDemo/joint_states`
の `ServoDemo`)、指令の方式 (JointState トピック / ros2_control)、非常停止トピック (空なら
`/<名前空間>/estop`)。ここで決めたものを ⑤ で確かめ、⑥ では変えません。

関節名と接続先が実機と合っていれば「次へ」。

### 接続先を実機に合わせる

3 つの値は実機側の作りで決まります。実機 (またはそのシミュレータ) を動かした状態で、ROS 2 の入った
端末から次を確かめてください。

| 決めるもの | 実機のどこを見るか | 確かめ方 |
|---|---|---|
| 名前空間 | 関節状態のトピック名。`/ServoDemo/joint_states` なら `ServoDemo`、`/joint_states` (前置き無し) なら空欄 | `ros2 topic list \| grep joint_states` |
| 指令の方式 | 関節を動かす側が何を待っているか。`sensor_msgs/JointState` を受けるドライバなら「JointState トピック」、ros2_control のコントローラがあるなら「ros2_control」 | `ros2 control list_controllers` (何も出なければ ros2_control ではない) |
| 非常停止トピック | 既存の停止信号があればその名前。無ければ空欄のまま (`/<名前空間>/estop` が使われます) | `ros2 topic list \| grep -i -e estop -e emergency` |

関節名は URDF のものがそのまま使われます。実機の `joint_states` に出る名前と 1 つでも違うと方策は
動きません。次で突き合わせてください。

```bash
ros2 topic echo /<名前空間>/joint_states --once --field name
```

**非常停止は空欄のままでも効きます。** 生成される方策ノードがそのトピックを購読し、`true` を受けると
指令を止めます。ただし止まるのは「方策が指令を出すこと」だけで、モータの電源は切れません。実機では
別に物理的な非常停止を用意してください。効いているかは、ロボットを浮かせた状態で次を送り、指令が
止まることで確かめます (⑤ でも自動で試験します)。

```bash
ros2 topic pub -1 /<名前空間>/estop std_msgs/Bool "{data: true}"    # 止まる
ros2 topic pub -1 /<名前空間>/estop std_msgs/Bool "{data: false}"   # 再開する
```

## ② タスク

「どこから始めて、どうなれば成功か」を決めます。固定基体のロボットなので「関節を目標角へ動かす」
タスクが選ばれ、右の 3D に各関節の目標範囲が緑の弧で描かれます。

- 開始の位置と向き: x, y [m] と向き [°]。ここで決めた姿勢が ① のプレビュー、③ の学習、⑤ のチェックの
  すべてで使われます (既定は向き 180°: 正面がカメラを向く)

![② タスク](images/tutorial2/02_task.png)

- 目標角の範囲: 自動 (可動範囲の 80 %) か、手で決める (スライダで ±rad)
- 成功とみなす誤差: 目標角から何 rad 以内か。ここでは 0.05 rad のまま
- 1 回の制限時間: 2 秒のまま
- 学習の目安: 許容誤差と並列数からの見込み。**許容誤差を半分にすると学習時間は数倍になります**

「次へ」で契約 (方策が何を見て何を出すかの定義) と学習設定が生成されます。契約 JSON を開く必要は
ありません (見たければ「詳細」)。

### 手先や物体を動かすタスク

固定基体では「何をさせるか」を切り替えられます。

- **手先を所定の場所へ**: 動かすリンクと、リンク上の点 (先端が既定)、目標の箱 (中心と大きさ、根リンク
  座標系 [m]) を決めます。3D には緑の箱と点 (緑の球) が出ます。目標は箱の中から抽選するので、
  届く範囲に置いてください。
- **物体を所定の場所へ**: 手先の姿勢や関節角は問わず、床の物体を緑の枠へ押し込めば成功です。
  物体 (箱・球・円柱、大きさ、質量)、押すリンク、物体の開始位置 (中心 ± ばらつき)、目標の枠
  (中心と大きさ) を決めます。3D には物体 (橙) と開始の枠 (橙)、目標の枠 (緑) が地面に描かれます。
  開始位置と目標の両方が押すリンクの届く範囲にあることを確かめてください。

![② 物体を所定の場所へ](images/tutorial2/02b_task_object.png)

物体のタスクでは ④ に「物体を開始位置へ戻す」ボタンと目標 (x, y) のスライダが出て、⑤ はシミュレータの
物体の位置を実機のカメラの代わりに使って判定します。実機では物体の位置を PoseStamped (ロボットの根
リンク座標系) で出すトピックが必要で、DEPLOY.md の接続表に行が増えます。

### 座標系と「大きさ」の読み方

数値の意味を取り違えると、届かない場所を目標にしてしまいます。

**座標系** は 2 つあります。どちらも ROS の向きで、x が前、y が左、z が上です (単位はすべて m、角度は
度で入力する欄だけ `°` と書いてあります)。

- **開始の位置と向き** の x, y は「世界の原点から見たロボットの置き場所」です。ロボット自身を動かします。
- **手先・物体・目標の枠** の x, y, z は「ロボットの根リンクから見た位置」です。ロボットを動かしても
  一緒に動きます。つまり `x = 0.3` は「ロボットの 0.3 m 前」であって、世界の x = 0.3 ではありません。

```
        y (左)
          ^          根リンク座標系。ロボットの向きに合わせて回ります
          |
   +------+------> x (前)     x = 0.3, y = 0.15 は「0.3 m 前、0.15 m 左」
          |
      (根リンク = URDF の一番根元のリンク。多くは base_link)
```

**「大きさ」が 3 か所に出てきます。** 別のものなので混ぜないでください。

| 欄 | 意味 | 例 |
|---|---|---|
| 物体の大きさ | 物体そのものの寸法 (箱なら縦横高さ、球なら半径) | `0.05` の箱 = 5 cm 角 |
| 開始位置 ± ばらつき | 毎回の開始位置を抽選する幅 (中心からの片側) | `±0.03` なら中心から 3 cm 以内 |
| 目標領域の大きさ | 目標の枠の一辺の長さ (中心からではなく全体) | `0.12` なら 12 cm 角の枠 |

**許容誤差は枠とは別です。** 枠は「目標の点をどこから抽選するか」、許容誤差は「その点にどれだけ
近づけば成功か」です。枠を広げても課題が易しくなるわけではなく (目標が散らばるので少し難しくなります)、
易しくするには許容誤差を広げます。

```
   ┌──────────────┐  ← 目標領域 (大きさ 0.12): この中から目標の点を毎回抽選
   │      ·   ●   │  ● = 今回の目標の点
   │        ( )   │  ( ) = 許容誤差 0.06: この中に入れば成功
   └──────────────┘
```

## ③ 学習

![③ 学習の前](images/tutorial2/03_train_ready.png)

「学習を始める」を押すと、右に 8 体のロボットが 3×3 の格子に並んで学習が始まります (それぞれ別の
目標で同時に試行しています)。1 秒ごとに進捗が文で出ます。

![③ 学習中](images/tutorial2/04_training.png)

「学習中 4% / 成功率 3% (直近 200 回) / 誤差 0.327 / 目標 0.05 / 残り およそ 22 分 / (目標に達すれば早く終わります) /
8 体並列」。残り時間は上限までの見込みで、目標の誤差に達すれば早く終わります。

グラフの読み方: 横軸は試行の順 (左が最初)。青の点は「その試行の終わりに目標からどれだけ離れていたか」で、
橙の線が「成功とみなす誤差」です。青の点がこの線より下に来る試行が増えると、緑の線 (直近の試行で成功した
割合、右端の目盛 0〜100 %) が上がります。学習が進むと青の点が橙の線の下に集まり、緑の線が上がっていきます。

### 報酬の内訳 (何が起きているか)

成功率と誤差だけでは「なぜ上がらないか」が分かりません。その下のグラフは、1 試行あたりに何で点を
取り、何で減点されているかを項目ごとに描いたものです。中央の線が 0 で、上が報酬、下が罰です。
凡例に、いま効いている順に項目名と 1 試行あたりの値が出ます (↑ ↓ は前の 200 試行との比較)。

![③ 報酬の内訳と学習器の内部](images/tutorial2/04b_train_breakdown.png)

読み方の例 (物体を押すタスク):

| 見えるもの | 起きていること | 次の一手 |
|---|---|---|
| 「手先と対象の距離」の罰が減らない | 手先が物体に近づけていない | ② で物体の開始位置を手先の届く範囲に寄せる |
| 手先は近づくが「目標へ近づいた分」が出ない | 触れてはいるが押せていない | 押すリンクを変える、物体を軽くする、制限時間を延ばす |
| 「動きの荒さ」の罰が一番大きい | 指令が暴れている | 制限時間を延ばす。それでも収まらなければ詳細 > Train の学習率を下げる |
| 「条件を満たせた回数」が伸びている | 成功が増えている | そのまま待つ |

「詳細」を押すと、右に**学習器の内部**が出ます。ここは学習そのものが健全かを見る場所です。

- **方策のばらつき**: 大きいほど色々試しています。早く 0 に近づくと探索をやめて頭打ちになります。
- **価値の説明率**: 「この状態はどれくらい良いか」の予測の当たり具合です。学習の序盤は低く、
  進んでも 0 付近のままなら、1 回の制限時間が長すぎるか報酬が疎すぎます。
- **更新の大きさ**: 1 回の更新で方策がどれだけ変わったかです。0.05 を超えると不安定なので、
  学習率を下げます。

これらが悪いときは「次の一手」にも出るので、数値を読めなくても構いません。

![③ 学習が終わった](images/tutorial2/05_trained.png)

この例では 7〜8 分で目標に達して止まりました (「学習が終わりました / 成功率 57% / 誤差 0.050 / 目標 0.05」)。
うまく行かないとき (成功率が 0 のまま、途中で悪化) は文の下に「次の一手」と「② に戻る」ボタンが出ます。

## ④ 試す (任意)

学習した方策をその場で動かします。「動かす」を押し、関節ごとのスライダで目標を変えると追従します。
状態は「誤差 0.002 rad / 経過 17 s」のように出ます。台車のロボットなら「地面をクリックして目標を置く」
で目標 (緑の円) を置け、走った軌跡が青い線で描かれます。

![④ 試す](images/tutorial2/06_try.png)

## ⑤ チェック (配備前)

実機に持っていく前に、この画面のシミュレータを相手に ROS 2 の経路で方策を動かして確かめます。
「チェックを実行」を押すと 5 段階が順に進みます (パッケージ生成 → ビルド → シミュレータに接続して
スポーン → ROS 2 経由で方策を動かして判定 → 完了)。1〜2 分かかります。

![⑤ 実行中](images/tutorial2/07_checking.png)

結果は平易なチェックリストです。非常停止の試験も含まれます。

![⑤ 合格](images/tutorial2/08_check.png)

NG があれば「次の一手」に何を直せばよいかが出ます (関節名の対応、行動の scale/offset、許容誤差、
シミュレータのフレームレート、など)。

ROS 2 が無い PC では、この画面の「コンテナを起動」で同梱のコンテナが立ち上がり、チェックはその中で
実行されます (プロジェクトのフォルダはリポジトリの配下にある必要があります)。

## ⑥ 実機へ

⑤ で確かめた契約と方策から ROS 2 パッケージと手順書 (DEPLOY.md) を作ります。接続先は ① で決めた
ものがそのまま使われます。

![⑥ 実機へ](images/tutorial2/09_deploy.png)

「作る」を押すと、出力先のフォルダ (既定はプロジェクトの `deploy/`) に ROS 2 のパッケージと
DEPLOY.md ができます。画面に要約 (接続するトピック、非常停止、上限値、起動コマンド) が出て、
「手順書を開く」で DEPLOY.md の全文が読めます。

### 実機へ持っていく

できたパッケージのフォルダ (`deploy/<名前>_policy`、`package.xml` がある階層) を、ロボット側の
colcon ワークスペースの `src/` にコピーします。ONNX の方策ファイルもパッケージの中に入っているので、
これ 1 つで足ります。

```bash
# 手元の PC から (<robot> はロボットのホスト名)
scp -r ~/unirobolab_projects/servo/deploy/servodemo_policy <robot>:~/colcon_ws/src/

# ロボット側で
cd ~/colcon_ws
colcon build --packages-select servodemo_policy
source install/setup.bash
pip install onnxruntime        # 方策ノードの Python 環境に無ければ
ros2 launch servodemo_policy policy.launch.py ns:=ServoDemo
```

ワークスペースが無ければ `mkdir -p ~/colcon_ws/src` から作れます。同じ PC で動かすなら scp は要らず、
`src/` へコピーするだけです。

起動すると方策ノードは `joint_states` を待ち、目標が届くまで指令を出しません。目標は
`/<名前空間>/policy/command` に流します (関節目標なら関節ごとの角度、地点や物体なら x, y, z)。

```bash
ros2 topic pub -1 /ServoDemo/policy/command std_msgs/Float64MultiArray "{data: [0.5, -0.5]}"
```

DEPLOY.md の「動かす前に確かめること」(非常停止、可動範囲と速度の上限、観測の鮮度、立ち上がり時間)
を順に確認してから動かしてください。最初はロボットを浮かせるか、可動範囲に何も無い状態で試します。

## 途中でやめて後で続けるとき

もう一度同じ起動コマンドを実行すると、前回のプロジェクトが開き、成果物の有無から続きの段階に
戻ります。① のロボットや接続先、② のタスクを変えると、それより後の段階に「!」が付き、やり直しが
必要なことが分かります。

## 困ったとき

| 症状 | 見るところ |
|---|---|
| 「学習を始める」が押せない | 下の状態行に理由が出ます (Python 環境が無い、学習サーバが無効) |
| 「試す」で誤差が大きいまま | ③ の学習結果が古くないか (ステッパーの「!」)。詳細の実行器ログ |
| ⑤ が実行できない | ROS 2 もコンテナも無い。`scripts/sim2sim_container.sh start` か「コンテナを起動」 |
| 日本語が表示されない | OS に日本語フォント (Noto Sans CJK など) が無いと英語表示になります |

## この版で残っている課題

- ① は関節名を表で直せません (今は表示だけ)。実機の関節名と URDF が違うときは URDF 側を直してください。
- ② の開始条件は「今の姿勢・位置」に固定です (v1)。
- ⑤ をこの PC の ROS 2 で回すには、シミュレータ側のユーティリティ (simulation_ros2_utils、ros_tcp_endpoint)
  が必要です。コンテナには入っています。
